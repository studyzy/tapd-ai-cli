...
- SDK：`github.com/studyzy/tapd-sdk-go`（同仓库 `sdk/` 子目录，独立 `go.mod`，仅依赖标准库）
...
sdk/            # TAPD SDK 独立模块（module github.com/studyzy/tapd-sdk-go）
  model/        # TAPD 数据模型（Workspace、Story、Task、Bug、Iteration 等）
  client.go     # Client 结构体、认证、HTTP 基础方法
  story.go      # Story CRUD 方法（其他资源同理）
internal/
...
1. **SDK 层（sdk/）**：独立 Go 模块，封装所有 TAPD API HTTP 调用，返回原始数据（含 HTML），仅依赖标准库，可被第三方独立引入。
...
- 每个重要的导出函数必须有单元测试
- SDK 层：使用 `net/http/httptest` 进行 mock server 测试（`cd sdk && go test ./...`）
- 命令层：测试参数解析和输出格式
