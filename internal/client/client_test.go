...
func TestNewClient_BearerAuth(t *testing.T) {
	...
	c := client.NewClientWithBaseURL(srv.URL, "my-token", "", "")
	err := c.TestAuth()
	if err != nil {
...
func TestNewClient_BasicAuth(t *testing.T) {
	...
	c := client.NewClientWithBaseURL(srv.URL, "", "user1", "pass1")
	err := c.TestAuth()
	if err != nil {
		t.Fatalf("TestAuth() unexpected error: %v", err)
	}
...
func TestNewClient_AccessTokenPreferred(t *testing.T) {
	...
	c := client.NewClientWithBaseURL(srv.URL, "my-token", "user1", "pass1")
	err := c.TestAuth()
	if err != nil {
		t.Fatalf("TestAuth() unexpected error: %v", err)
	}
...
func TestTestAuth_Success(t *testing.T) {
	...
	c := client.NewClientWithBaseURL(srv.URL, "token", "", "")
	err := c.TestAuth()
	if err != nil {
...
func TestTestAuth_Failure(t *testing.T) {
	...
	c := client.NewClientWithBaseURL(srv.URL, "bad-token", "", "")
	err := c.TestAuth()
	if err == nil {
		t.Fatal("TestAuth() expected error for status=0, got nil")
	}
	var tapdErr *client.TAPDError
	if !errors.As(err, &tapdErr) {
		t.Fatalf("expected *client.TAPDError, got %T", err)
	}
